...
export default function Editor({ templateId, initial }) {
  ...
  const updateTemplateStatus = async (newStatus) => {
    try {
      ...
      const res = await axios.patch(
        `${baseURL}/template/status/${templateId}`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      router.push(`/published/${templateId}`);
      alert(`Template ${newStatus} successfully ✅`);
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.put(
        `${baseURL}/template/update/${templateId}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      <Header
        ...
        rightContent={
          <>
            <Button
              disabled
              className={`px-4 py-2 text-sm font-semibold cursor-default
                ${
                  status === "published"
            ...
            {status !== "published" && (
              <Button onClick={() => updateTemplateStatus("published")}>
                Publish
              </Button>
            )}
          </>
      ...
      <div className="flex h-[calc(100vh-80px)]">
        ...
        <div className="w-1/3 bg-[#242424] p-6 overflow-y-auto">
          ...
          {/* Section Picker */}
          <section className="p-4 border rounded-md mb-4">
            <h3 className="font-bold mb-4 text-2xl">Sections</h3>
          ...
          <div className="space-y-4">
            ...
            {/* Contact Us */}
            {websiteData.sections.contactUs && (
              <div className="mt-8 rounded-lg space-y-2">
            ...
            {websiteData.sections.footer && (
              <div className="mt-8 rounded-lg space-y-2">
                ...
                <input
                  ...
                  value={websiteData.footer.brandLogo}
                  onChange={(e) =>
                    setWebsiteData({
                      ...websiteData,
